        ...
        {showLens && (
          <div
            ...
            style={{
              ...
              backgroundPosition: `${((lensPos.x / (wrapRef.current?.clientWidth || 1)) * 100) * 1.2}% ${((lensPos.y / (wrapRef.current?.clientHeight || 1)) * 100) * 1.2}%`,
            }}
          />
        )}
      ...
      {isAdminUser && (
        <div className="mt-8 border-t pt-6">
          ...
          <p className="text-sm text-gray-600 mb-4">
            Quick actions for flagged reviews. Approve to mark good reviews, or
            Unflag to clear flags.
          </p>

          {reviews.filter((r) => r.flagged || r.flags > 0).length === 0 ? (
            <div className="text-sm text-gray-500">
              No flagged reviews currently.
            </div>
          ) : (
...
export default function ProductDetailPage() {
      ...
      const items = snap.docs.map((d) => {
        ...
        return {
          ...
          createdAt: raw.createdAt
            ? raw.createdAt.toMillis?.() ?? raw.createdAt
            : null,
  ...
  const mainImage = useMemo(
    () => (images.length ? images[active]?.url : "/products/placeholder.png"),
    [images, active]
  );
  ...
  const gold = useMemo(
    () =>
      product?.priceModel === "gold"
        ? goldPrice(product, pricingSettings || {})
        : null,
  ...
  /* -------------------- Review Actions (with Admin) -------------------- */
  const isAdminUser =
    user && ["mohitkmr8566@gmail.com", "warea.admin@gmail.com"].includes(user.email);
  ...
  }, [user, reviewRating, reviewComment, id, fetchReviews]);
  ...
  const handleReportReview = useCallback(
    async (reviewId) => {
      ...
      try {
        ...
        await runTransaction(db, async (tx) => {
          ...
          tx.update(reviewRef, { flagged: true, flags: curFlags + 1 });
        });
        // optional notify
        ...
        toast.success("Thank you for reporting. We'll review it soon.");
        await fetchReviews();
      } catch (err) {
    ...
    [user, id, product?.title, fetchReviews]
  );
  ...
  const handleApproveReview = useCallback(
    async (reviewId) => {
      ...
      try {
        ...
        const reviewRef = doc(db, "products", id, "reviews", reviewId);
        await updateDoc(reviewRef, { approved: true, flagged: false, flags: 0 });
  ...
  const handleUnflagReview = useCallback(
    async (reviewId) => {
      ...
      try {
        if (auth?.currentUser) await auth.currentUser.getIdToken(true);
        const reviewRef = doc(db, "products", id, "reviews", reviewId);
        await updateDoc(reviewRef, { flagged: false, flags: 0 });
  ...
  const desc =
    ...
    `Shop ${product.title} by Warea. Anti-tarnish, Premium handcrafted jewellery with fast delivery.`;
  const ogImg = images?.[0]?.url || `${baseUrl}/og-image.jpg`;

  const schemas = getProductPageSchemas(
    baseUrl,
    product || {},
    reviews || [],
    ratingValue,
  ...
  return (
    <>
          ...
          <div className="space-y-6">
            ...
            {product.priceModel === "gold" ? (
              <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                ...
                <p className="mt-2 text-xs text-gray-500">
                  Rate ₹{fmtINR(pricingSettings?.goldRatePerGram || 0)}/g • Weight{" "}
            ...
            <div className="bg-white border border-gray-200 rounded-xl p-4">
              ...
              <form onSubmit={checkPincode} className="flex gap-2">
                ...
                <button
                  type="submit"
                  className="px-4 py-2 rounded-lg text-sm bg-amber-500 text-gray-900 hover:bg-amber-400"
                >
                  Check
                </button>
              </form>
      ...
      {/* Sticky Mobile CTA — hidden if fullscreen */}
      <StickyMobileCTA
